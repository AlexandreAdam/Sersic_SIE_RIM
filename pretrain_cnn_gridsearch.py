...
def uniform_grid_search(args):
    for gridsearch_id in range(1, args.n_models + 1):
        ...
        nicknames = []
        params = []
        for p in EXTRA_PARAMS + CNN_MODEL_HPARAMS :
...
def distributed_strategy(args):
    ...
    for gridsearch_id in range((THIS_WORKER - 1), len(gridsearch_args), N_WORKERS):
        ...
        params_dict = {k: v for k, v in vars(run_args).items() if k in EXTRA_PARAMS + CNN_MODEL_HPARAMS }
        params_dict.update({
        ...
        grid_csv_path = os.path.join(os.getenv("ARIM"), "results", f"{args.logname_prefixe}.csv")
        this_run_csv_path = os.path.join(os.getenv("ARIM"), "results", f"{run_args.logname}.csv")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cnn_filters",                default=32, nargs="+",      type=int)
    parser.add_argument("--cnn_activation",             default="relu", nargs="+")
    ...
    parser.add_argument("--initial_learning_rate",  default=1e-6,   nargs="+",   type=float,     help="Initial learning rate.")
